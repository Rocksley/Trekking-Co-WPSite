					...
					<a href="<?php the_permalink(); ?>">
						<div class="icon-box style1 fourty-space">
							<?php if ( $post_type == 'trek' ) { ?>
								...
								<p class="box-title"><?php the_title(); ?> in <span class="price"><?php echo esc_html( trav_get_price_field( get_post_meta( $the_query->post->ID, 'trav_trek_avg_price', true ) ) ) ?></span></p>
				...
				<?php
					...
					if ( ! empty( $recent_activity ) ) {
						...
						foreach ( $recent_activity_array as $post_id ) {
							...
							if ( $post_type == 'trek' ) {
								$_country = get_post_meta( $post_id, 'trav_trek_country', true );
								if ( ! empty( $_country ) ) {
									if ( $country_obj = get_term_by( 'id', $_country, 'location' ) ) $_country = __( $country_obj->name, 'trav');
								}

								$_city = get_post_meta( $post_id, 'trav_trek_city', true );
								if ( ! empty( $_city ) ) {
								...
								<li>
									<a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>">
										...
										<span class="price"><small><?php _e( 'avg/night', 'trav' ); ?></small><?php echo esc_html( trav_get_price_field( get_post_meta( $post_id, 'trav_trek_avg_price', true ) ) ); ?></span>
