<?php
/**
 * Trek Booking Confirmation Template
 */
global $wpdb;
...
$acc_id = trav_acc_clang_id( $booking_data['trek_id'] );
$room_type_id = trav_room_clang_id( $booking_data['room_type_id'] );
$deposit_rate = get_post_meta( $acc_id, 'trav_trek_security_deposit', true ); $deposit_rate = empty( $deposit_rate ) ? 0 : $deposit_rate;
...
if ( ! empty( $deposit_rate ) && empty( $booking_data['deposit_paid'] ) ) {
	...
	if ( $payment_result ) {
		if ( ! empty( $payment_result['success'] ) && ( $payment_result['method'] == 'paypal' ) ) {
			...
			$update_status = $wpdb->update( TRAV_TREK_BOOKINGS_TABLE, array( 'deposit_paid' => $booking_data['deposit_paid'], 'status' => 1, 'other' => serialize( $other_booking_data ) ), array( 'booking_no' => $booking_data['booking_no'], 'pin_code' => $booking_data['pin_code'] ) );
...
if ( $booking_data['status'] == 1 ) { // if upcoming 
	trav_get_template( 'booking-success.php', '/templates/trek/' ); 
} elseif ( $booking_data['status'] == 0 ) { // if canceled 
	trav_get_template( 'booking-cancelled.php', '/templates/trek/' ); 
} elseif ( $booking_data['status'] == 2 ) { // if completed 
	trav_get_template( 'booking-completed.php', '/templates/trek/' ); 
}