...
$tax_rate = get_post_meta( $acc_id, 'trav_trek_tax_rate', true );
?>
...
<div class="row">
	<div class="col-sm-8 col-md-9">
		<div class="booking-information travelo-box">
			...
			<?php trav_get_template( 'acc-room-detail.php', '/templates/trek/' ); ?>
			<?php do_action( 'trav_acc_conf_form_after', $booking_data ); ?>
		</div>
	</div>
	<div class="sidebar col-sm-4 col-md-3">
		<?php if ( empty( $acc_meta["trav_trek_d_edit_booking"] ) || empty( $acc_meta["trav_trek_d_edit_booking"][0] ) || empty( $acc_meta["trav_trek_d_cancel_booking"] ) || empty( $acc_meta["trav_trek_d_cancel_booking"][0] ) ) { ?>
			<div class="travelo-box edit-booking">
				...
				<ul class="triangle hover box">
					<?php if ( empty( $acc_meta["trav_trek_d_edit_booking"] ) || empty( $acc_meta["trav_trek_d_edit_booking"][0] ) ) { ?>
					...
					<?php } ?>
					<?php if ( empty( $acc_meta["trav_trek_d_cancel_booking"] ) || empty( $acc_meta["trav_trek_d_cancel_booking"][0] ) ) { ?>
...
<script>
	...
	tjq(document).ready(function(){
		...
		tjq('body').on('change', '#change-date-form input, #change-date-form select', function(){
			...
			<?php if ( ! empty( $acc_meta["trav_trek_minimum_stay"] ) ) { ?>

				minimum_stay = <?php echo esc_js( $acc_meta["trav_trek_minimum_stay"][0] )?>;
				if (date_from + one_day * minimum_stay - date_to > 0) { alert('<?php echo sprintf( __( 'Minimum stay for this trek is %d nights. Have another look at your dates and try again.', 'trav' ), $acc_meta["trav_trek_minimum_stay"][0] ) ?>'); return false; }
