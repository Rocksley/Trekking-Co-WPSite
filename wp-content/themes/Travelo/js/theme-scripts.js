...
function trav_init_chasermenu() {
	if (enableChaser == 1 && tjq('#content').length > 0 && tjq('#main-menu ul.menu').length > 0) {
		var forchBottom;
		var chaser = tjq('#main-menu ul.menu').clone().hide().appendTo(document.body).wrap("<div class='chaser hidden-mobile' id='chaser'><div class='container'></div></div>");
...
/* Trek Search Page */
// trek type filter
tjq("#trek-type-filter .filters-option li").click(function(){
	var url_noacc_type = tjq("#trek-type-filter").data('url-noacc_type').replace(/&amp;/g, '&');
	if (tjq(this).hasClass('all-types')) {
		if (! tjq(this).hasClass('active')) {
			tjq(this).toggleClass('active');
			return false;
		} else {
			tjq("#trek-type-filter .filters-option li").removeClass('active');
			tjq(this).addClass('active');
		}
	} else {
		if (tjq("#trek-type-filter .filters-option li.active").length == 0) {
			tjq("#trek-type-filter .filters-option li.all-types").addClass('active');
		} else {
			tjq("#trek-type-filter .filters-option li.all-types").removeClass('active');
			tjq("#trek-type-filter .filters-option li.active").each(function(index){
...
// make trek search works with valid date frame
