		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				if ( isset( $attributes['trav-booking-no'] ) && isset( $attributes['trav-pin-code'] ) ) {
					...
					$property_type = '';
					if ( has_term( 'trek', 'product_cat', $product_id ) ) {
						$property_type = 'trek';
				...
				?>
					<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="product-thumbnail">
							<?php if ( $property_type == 'trek' ) { ?>
								<a href="<?php echo get_permalink( $booking_data['trek_id'] ); ?>">
									<?php echo get_the_post_thumbnail( $booking_data['trek_id'], 'thumbnail' ); ?>
								</a>
						...
						<td class="product-name">
							<?php if ( $property_type == 'trek' ) { ?>
								<h5 class="product-title">
									<?php echo get_the_title( $booking_data['trek_id'] ); ?>
