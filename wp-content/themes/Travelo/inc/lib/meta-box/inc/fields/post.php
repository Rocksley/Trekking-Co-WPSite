...
if ( !class_exists( 'RWMB_Post_Field' ) )
{
	class RWMB_Post_Field extends RWMB_Field
	{
		...
		{
			/* To show things to do in same location */
			if ( isset( $field['post_type'] ) && ( $field['post_type'] == 'things_to_do' ) ) {
				$post_id = '';
				if ( isset( $_GET['post'] ) ) {
					$post_id = $_GET['post'];
					$city = get_post_meta( $post_id, 'trav_trek_city', true );
					if ( ! empty( $city ) ) {
						$field['query_args']['tax_query'] = array(
							array(
								'taxonomy' => 'location',
								'terms'    => $city,
							),
						...
						unset($field['query_args']['meta_query']);
					}
				} else {
					unset($field['query_args']['meta_query']);
				}
			...
			/* To show own treks only for select box */
			if ( ! current_user_can( 'manage_options' ) ) {
				if ( isset( $field['post_type'] ) && ( $field['post_type'] == 'trek' ) ) {
