...
if ( ! function_exists( 'trav_enqueue_scripts' ) ) {
	function trav_enqueue_scripts() {
		...
		wp_register_script( 'trav_script_trek', TRAV_TEMPLATE_DIRECTORY_URI . '/js/trek.js', array( 'jquery' ), '', true );
		...
		if ( is_singular( 'trek' ) ) {
			...
			wp_localize_script( 'trav_script_trek', 'acc_data', $acc_data );
			wp_enqueue_script( 'trav_script_trek' );
...
if ( ! function_exists( 'trav_template_chooser' ) ) {
	function trav_template_chooser($template) {
		...
		if ( is_search() && $post_type == 'trek' ) {
			// return locate_template( 'templates/trek/search-trek.php' );
			return locate_template( 'archive-trek.php' );
...
if ( ! function_exists( 'trav_posts_request_filter' ) ) {
	function trav_posts_request_filter( $input ) {
		global $wp_query;
		if ( $wp_query->is_main_query() && is_search() && $wp_query->get( 'post_type' ) == 'trek' ) {
			//return false;
		}
		return $input;
	}
...
if ( ! function_exists( 'trav_remove_admin_bar' ) ) {
	function trav_remove_admin_bar() {
		if ( ! current_user_can( 'edit_treks' ) && ! is_admin() ) {
		...
		if ( is_home() ) {}
			...
			if( is_single() ) {
				if ( ( $post->post_type == 'post' ) ) {
					...
					// other single post breadcrumb - trek etc
					echo '<li class="active">' . esc_html( get_the_title() ) . '</li>';
				}
			}
...
if ( ! function_exists( 'trav_post_get_location' ) ) {
	function trav_post_get_location( $post_id, $post_type='trek', $location_type='city' ) {
...
/* to make property owner can be set as trek author */
...
if ( ! function_exists( 'trav_get_available_modules' ) ) {
	function trav_get_available_modules() {
		global $trav_options;
		$modules = array();
		if ( empty( $trav_options['disable_acc'] ) ) $modules[] = 'trek';
...
if ( ! function_exists( 'trav_get_user_booking_list' ) ) {
	function trav_get_user_booking_list( $user_id, $status = 1, $sortby = 'created', $order='desc' ) {
		...
		// $sql = $wpdb->prepare( 'SELECT * FROM ' . TRAV_TREK_BOOKINGS_TABLE . $where . $order_by, $user_id );
		$available_modules = trav_get_available_modules();
		$sqls = array();
		if ( in_array( 'trek', $available_modules ) ) {
			$sqls[] = "SELECT 'trek' AS post_type, booking_no, pin_code, total_price, created, status, trek_id AS post_id, date_from AS event_date, adults, rooms AS tickets FROM " . TRAV_TREK_BOOKINGS_TABLE . $where;
		}
		...
		foreach ( $booking_list as $booking_data ) {
			...
			$conf_url = '';
			$icon_class = '';
			if ( 'trek' == $booking_data->post_type ) {
		...
		if ( ! empty( $_GET['install_trav_pages'] ) ) {
			...
			if ( empty( $installed ) ) {
				...
				if ( empty( $trav_options['acc_booking_page'] ) ) {
					$postarr = array(
						'post_title'    => 'Trek Booking',
						'post_type'     => 'page',
						'post_content'  => '[trek_booking]',
						'post_status'   => 'publish'
					);
				...
				if ( empty( $trav_options['acc_booking_confirmation_page'] ) ) {
					$postarr = array(
						'post_title'    => 'Trek Booking Confirmation',
						'post_type'     => 'page',
						'post_content'  => '[trek_booking_confirmation]',
						'post_status'   => 'publish'
					);
