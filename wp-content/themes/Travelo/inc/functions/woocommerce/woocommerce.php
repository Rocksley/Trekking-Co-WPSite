...
/*
 * Handle trek woocommerce booking payment
 */
if ( ! function_exists( 'trav_woo_add_acc_booking' ) ) {
	function trav_woo_add_acc_booking( $booking_data ) {
		...
		$product_name .= get_the_title( $booking_data['trek_id'] ) . ' ' . get_the_title( $booking_data['room_type_id'] ) . ' ' . $booking_data['rooms'] . __( 'rooms', 'trav' ) . ' ' . $date_from . ' - ' . $date_to;
		$product_content = __( 'From', 'trav' ) . ' ' . $date_from . ' ' . __( 'To', 'trav' ) . ' ' . $date_to . ' ' . get_the_title( $booking_data['trek_id'] ) . ' ' . get_the_title( $booking_data['room_type_id'] ) . ' ' . $booking_data['rooms'] . __( 'rooms', 'trav' );
		$booking_no = $booking_data['booking_no'];
		$pin_code = $booking_data['pin_code'];
		$booking_price = $booking_data['deposit_price'];
		$product_category_slug = 'trek';
