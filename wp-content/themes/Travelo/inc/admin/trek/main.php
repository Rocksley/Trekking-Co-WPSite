...
require_once( TRAV_INC_DIR . '/admin/trek/vacancies-admin-panel.php' );
require_once( TRAV_INC_DIR . '/admin/trek/bookings-admin-panel.php' );
require_once( TRAV_INC_DIR . '/admin/trek/reviews-admin-panel.php' );

/*
 * get trek room list from trek id
 */
if ( ! function_exists( 'trav_ajax_acc_get_acc_room_list' ) ) {
	function trav_ajax_acc_get_acc_room_list() {
		...
		if ( ! empty( $_POST['trek_id'] ) ) {
			$trek_id = trav_acc_clang_id( $_POST['trek_id'] );
			$args['meta_query'] = array(
					array(
						'key'     => 'trav_room_trek',
						'value'   => sanitize_text_field( $trek_id ),
					),
				);
		}
...
/*
 * get trek id from room type id
 */
if ( ! function_exists( 'trav_ajax_acc_get_room_acc_id' ) ) {
	function trav_ajax_acc_get_room_acc_id() {
		if ( isset( $_POST['room_id'] ) ) {
			$acc_id = get_post_meta( sanitize_text_field( $_POST['room_id'] ), 'trav_room_trek', true );
			echo esc_js( $acc_id );
		} else {
			//
		}
...
/*
 * add trek filter to admin/room_type list
 */
if ( ! function_exists('trav_acc_table_filtering') ) {
	function trav_acc_table_filtering() {
		...
		if ( isset( $_GET['post_type'] ) && 'room_type' == $_GET['post_type'] ) {
			$accs = get_posts( array( 'post_type'=>'trek', 'posts_per_page'=>-1, 'orderby'=>'post_title', 'order'=>'ASC', 'suppress_filters'=>0 ) );
			echo '<select name="acc_id">';
			echo '<option value="">' . esc_html__( 'All Treks', 'trav' ) . '</option>';
			foreach( $accs as $acc ) {
...
/*
 * Modify columns on admin/Trek list
 */
if ( ! function_exists('trav_acc_custom_columns') ) {
	function trav_acc_custom_columns( $column, $post_id ) {
		switch ( $column ) {
			...
			case 'location' :
				$city = get_post_meta( $post_id, 'trav_trek_city', true );
				$country = get_post_meta( $post_id, 'trav_trek_country', true );
				if ( ! empty( $city ) ) {
...
/*
 * remove or add columns on admin/Trek list
 */
...
if ( ! function_exists('trav_room_type_custom_columns') ) {
	function trav_room_type_custom_columns( $column, $post_id ) {
		switch ( $column ) {
			...
			case 'trek' :
				$acc_id = get_post_meta( $post_id, 'trav_room_trek', true );
				if ( ! empty( $acc_id ) ) {
...
/*
 * make trek column orderable on admin/room_type list
 */
if ( ! function_exists('trav_room_type_acc_column_orderby') ) {
	function trav_room_type_acc_column_orderby( $vars ) {
		if ( isset( $vars['orderby'] ) && 'room_type' == $vars['orderby'] && isset( $vars['orderby'] ) && 'trek' == $vars['orderby'] ) {
			$vars = array_merge( $vars, array(
				'meta_key' => 'trav_room_trek',
				'orderby' => 'meta_value'
			) );
		}
...
if ( ! function_exists( 'trav_acc_admin_enqueue' ) ) {
	function trav_acc_admin_enqueue($hook) {
		if ( 'post.php' == $hook || 'post-new.php' == $hook) {
			global $post_type;
			if ( $post_type == 'trek' ) {
				wp_enqueue_script( 'trav_admin_acc_admin_js', TRAV_TEMPLATE_DIRECTORY_URI . '/inc/admin/trek/js/admin.js' );
			}
		}
	}
}
...
if ( ! function_exists( 'trav_acc_remove_pending_booking' ) ) {
	function trav_acc_remove_pending_booking( ) {
		...
		$wpdb->query( "UPDATE " . TRAV_TREK_BOOKINGS_TABLE . " SET status = 0 WHERE status = 1 AND deposit_paid = 0 AND deposit_price > 0 AND created < '" . $check_time . "'" );
	}
}
...
if ( ! function_exists( 'trav_acc_change_booking_status' ) ) {
	function trav_acc_change_booking_status( ) {
		...
		$wpdb->query( "UPDATE " . TRAV_TREK_BOOKINGS_TABLE . " SET status = 2" . $where );
	}
}

/*
 * update meta value when trek save
 */
if ( ! function_exists( 'trav_init_acc_meta' ) ) {
	function trav_init_acc_meta( $post_id ) {
		if ( 'trek' == get_post_type( $post_id ) ) {
			$avg_price = get_post_meta( $post_id, 'trav_trek_avg_price', true );
			if ( '' == $avg_price ) {
				delete_post_meta( $post_id, 'trav_trek_avg_price' );
				add_post_meta( $post_id, 'trav_trek_avg_price', 0 );
			}
...
add_action( 'manage_trek_posts_custom_column' , 'trav_acc_custom_columns', 10, 2 );
...
add_filter( 'manage_trek_posts_columns', 'trav_acc_set_columns' );
