...
class Trav_Acc_Vacancy_List_Table extends WP_List_Table {
	...
	function column_trek_name( $item ) {
		return '<a href="' . get_edit_post_link( $item['acc_id'] ) . '">' . $item['trek_name'] . '</a>';
	}
	...
	function get_sortable_columns() {
		$sortable_columns = array(
			...
			'trek_name'    => array( 'trek_name', false ),
	...
	function process_bulk_action() {
		...
		if ( 'bulk_delete'===$this->current_action() ) {
			...
			if ( current_user_can( 'manage_options' ) ) {
				$sql = sprintf('DELETE FROM %1$s WHERE id IN (%2$s)', TRAV_TREK_VACANCIES_TABLE, "$format" );
			} else {
				$sql = sprintf('DELETE %1$s FROM %1$s INNER JOIN %2$s as trek ON trek_id=trek.ID WHERE %1$s.id IN (%3$s) AND trek.post_author = %4$d', TRAV_TREK_VACANCIES_TABLE, $post_table_name, "$format", $current_user_id );
			}

			$wpdb->query( $wpdb->prepare( $sql, $selected_ids ) );
			wp_redirect( admin_url( 'edit.php?post_type=trek&page=vacancies&bulk_delete=true') );
		}
	...
	function prepare_items() {
		...
		$where = "1=1";
		if ( ! empty( $_REQUEST['trek_id'] ) ) $where .= " AND Trav_Vacancies.trek_id = '" . esc_sql( trav_acc_org_id( $_REQUEST['trek_id'] ) ) . "'";
		...
		if ( ! current_user_can( 'manage_options' ) ) { $where .= " AND trek.post_author = '" . get_current_user_id() . "' "; }

		$sql = $wpdb->prepare( 'SELECT Trav_Vacancies.* , trek.ID as acc_id, trek.post_title as trek_name, room_type.ID as room_type_id, room_type.post_title as room_type_name FROM %1$s as Trav_Vacancies
						INNER JOIN %2$s as trek ON Trav_Vacancies.trek_id=trek.ID
						INNER JOIN %2$s as room_type ON Trav_Vacancies.room_type_id=room_type.ID
						WHERE ' . $where . ' ORDER BY %4$s %5$s
						LIMIT %6$s, %7$s' , TRAV_TREK_VACANCIES_TABLE, $post_table_name, '', $orderby, $order, ( $per_page * ( $current_page - 1 ) ), $per_page );

		$data = $wpdb->get_results( $sql, ARRAY_A );

		$sql = sprintf( 'SELECT COUNT(*) FROM %1$s as Trav_Vacancies INNER JOIN %2$s as trek ON Trav_Vacancies.trek_id=trek.ID WHERE %3$s' , TRAV_TREK_VACANCIES_TABLE, $post_table_name, $where );
...
if ( ! function_exists( 'trav_acc_vacancy_add_menu_items' ) ) {
	function trav_acc_vacancy_add_menu_items() {
		$page = add_submenu_page( 'edit.php?post_type=trek', 'Trek Vacancies', 'Vacancies', 'edit_treks', 'vacancies', 'trav_acc_vacancy_render_pages' );
...
if ( ! function_exists( 'trav_acc_vacancy_render_list_page' ) ) {
	function trav_acc_vacancy_render_list_page() {
		...
		<div class="wrap">
			<h2>Trek Vacancies <a href="edit.php?post_type=trek&amp;page=vacancies&amp;action=add" class="add-new-h2">Add New</a></h2>
			<?php if ( isset( $_REQUEST['bulk_delete'] ) ) echo '<div id="message" class="updated below-h2"><p>Vacancies deleted</p></div>'?>
			<select id="trek_filter">
				<option></option>
				<?php
				$args = array(
						'post_type'         => 'trek',
				...
				$trek_query = new WP_Query( $args );

				if ( $trek_query->have_posts() ) {
					while ( $trek_query->have_posts() ) {
						$trek_query->the_post();
						$selected = '';
						$id = $trek_query->post->ID;
						if ( ! empty( $_REQUEST['trek_id'] ) && ( $_REQUEST['trek_id'] == $id ) ) $selected = ' selected ';
			...
			<select id="room_type_filter">
				...
				<?php
					...
					if ( ! empty( $_REQUEST['trek_id'] ) ) {
						$args['meta_query'] = array(
								array(
									'key'     => 'trav_room_trek',
									'value'   => sanitize_text_field( $_REQUEST['trek_id'] ),
								),
							);
					}
			...
			<a href="edit.php?post_type=trek&amp;page=vacancies" class="button-secondary">Show All</a>
...
if ( ! function_exists( 'trav_acc_vacancy_render_manage_page' ) ) {
	function trav_acc_vacancy_render_manage_page() {
		...
		if ( 'add' == $_REQUEST['action'] ) {
			$page_title = "Add New Trek Vacancy";
		} elseif ( 'edit' == $_REQUEST['action'] ) {
			$page_title = 'Edit Trek Vacancy<a href="edit.php?post_type=trek&amp;page=vacancies&amp;action=add" class="add-new-h2">Add New</a>';
			...
			$sql = $wpdb->prepare( 'SELECT Trav_Vacancies.* , trek.post_title as trek_name, room_type.post_title as room_type_name FROM %1$s as Trav_Vacancies
							INNER JOIN %2$s as trek ON Trav_Vacancies.trek_id=trek.ID
							INNER JOIN %2$s as room_type ON Trav_Vacancies.room_type_id=room_type.ID
							WHERE ' . $where , TRAV_TREK_VACANCIES_TABLE, $post_table_name, $vacancy_id );
		...
		<div class="wrap">
			...
			<form method="post" onsubmit="return manage_vacancy_validateForm();">
								...
								<?php
									...
									if ( $trek_query->have_posts() ) {
										while ( $trek_query->have_posts() ) {
											...
											$selected = '';
											$id = $trek_query->post->ID;
											if ( ( ! empty( $vacancy_data['trek_id'] ) ) && ( $vacancy_data['trek_id'] == trav_acc_org_id( $id ) ) ) $selected = ' selected ';
						...
						<td>
							<select name="room_type_id" id="room_type_id">
								...
								<?php
									...
									if ( ! empty( $vacancy_data['trek_id'] ) ) {
										$args['meta_query'] = array(
												array(
													'key'     => 'trav_room_trek',
													'value'   => trav_acc_clang_id( $vacancy_data['trek_id'] ),
												),
											);
									}
									$room_type_query = new WP_Query( $args );
				...
				<a href="edit.php?post_type=trek&amp;page=vacancies" class="button-secondary">Cancel</a>
...
if ( ! function_exists( 'trav_acc_vacancy_delete_action' ) ) {
	function trav_acc_vacancy_delete_action() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			$sql = $wpdb->prepare( 'SELECT Trav_Vacancies.trek_id FROM %1$s as Trav_Vacancies WHERE Trav_Vacancies.id = %2$d' , TRAV_TREK_VACANCIES_TABLE, $_REQUEST['vacancy_id'] );
		...
		// do action
		$wpdb->delete( TRAV_TREK_VACANCIES_TABLE, array( 'id' => $_REQUEST['vacancy_id'] ) );
		//}
		wp_redirect( admin_url( 'edit.php?post_type=trek&page=vacancies') );
		exit;
	}
}
	...
	function trav_acc_vacancy_save_action() {
		...
		if ( ! isset( $_POST['vacancy_save'] ) || ! wp_verify_nonce( $_POST['vacancy_save'], 'trav_acc_vacancy_manage' ) ) {
		   ...
		   exit;
			...
			$default_vacancy_data = array( 'trek_id'  => '',
										'room_type_id'      => '',
										'rooms'        => 0,
										'date_from'        => date( 'Y-m-d' ),
										'date_to'          => '9999-12-31',
			...
			$table_fields = array( 'date_from', 'date_to', 'trek_id', 'room_type_id', 'rooms', 'price_per_room', 'price_per_person' );
			$data = array();
			...
			$data['trek_id'] = trav_acc_org_id( $data['trek_id'] );
			$data['room_type_id'] = trav_room_org_id( $data['room_type_id'] );
			if ( empty( $_POST['id'] ) ) {
				//insert
				$wpdb->insert( TRAV_TREK_VACANCIES_TABLE, $data );
				$id = $wpdb->insert_id;
			} else {
				//update
				$wpdb->update( TRAV_TREK_VACANCIES_TABLE, $data, array( 'id' => sanitize_text_field( $_POST['id'] ) ) );
				$id = sanitize_text_field( $_POST['id'] );
			}
			wp_redirect( admin_url( 'edit.php?post_type=trek&page=vacancies&action=edit&vacancy_id=' . $id . '&updated=true') );
			exit;
		}
	}
}
...
if ( ! function_exists( 'trav_acc_vacancy_admin_enqueue_scripts' ) ) {
	function trav_acc_vacancy_admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'trav_admin_acc_script' , TRAV_TEMPLATE_DIRECTORY_URI . '/inc/admin/trek/js/script.js', array('jquery'), '1.0', true );
	}
}
