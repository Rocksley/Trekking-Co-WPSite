...
class Trav_Review_List_Table extends WP_List_Table {
	...
	function column_author_info( $item ) {
		...
		$str .= '<a href="edit.php?post_type=trek&amp;page=reviews&amp;reviewer_ip=' . esc_url( $item['reviewer_ip'] ) . '">' . $item['reviewer_ip'] . '</a></span>';
		return $str;
	}
	...
	function column_trek_name( $item ) {
		return '<a href="' . get_edit_post_link( $item['post_id'] ) . '">' . $item['trek_name'] . '</a>';
	}
	...
	function process_bulk_action() {
		...
		switch( $this->current_action() ) {
				//wp_redirect( admin_url( 'edit.php?post_type=trek&page=reviews&bulk_delete=true') );
	...
	function prepare_items() {
		...
		$where = "1=1";
		if ( ! empty( $_REQUEST['trek_id'] ) ) $where .= " AND Trav_Reviews.post_id = '" . esc_sql( trav_acc_org_id( $_REQUEST['trek_id'] ) ) . "'";
		...
		$sql = $wpdb->prepare( 'SELECT Trav_Reviews.* , trek.post_title as trek_name FROM %1$s as Trav_Reviews
						INNER JOIN %2$s as trek ON Trav_Reviews.post_id=trek.ID
...
if ( ! function_exists( 'trav_review_add_menu_items' ) ) {
	function trav_review_add_menu_items() {
		$page = add_submenu_page( 'edit.php?post_type=trek', 'Trek Reviews', 'Reviews', 'manage_options', 'reviews', 'trav_review_render_pages' );
...
if ( ! function_exists( 'trav_review_render_list_page' ) ) {
	function trav_review_render_list_page() {
		...
		$page_url = 'edit.php?post_type=trek&page=reviews';
		?>

		<div class="wrap">
			
			<h2><?php _e( 'Trek Reviews', 'trav' )?><a href="<?php echo esc_url( $page_url ); ?>&amp;action=add" class="add-new-h2">Add New</a></h2>
		...
		jQuery(document).ready(function($) {
			...
			$('#trek_filter').change(function() {
				...
				if (trekId) loc_url += '&trek_id=' + trekId + '&status=' + status;
...
if ( ! function_exists( 'trav_review_render_manage_page' ) ) {
	function trav_review_render_manage_page() {
		...
		if ( 'add' == $_REQUEST['action'] ) {
			$page_title = "Add New Trek Review";
		} elseif ( 'edit' == $_REQUEST['action'] ) {
			$page_title = 'Edit Trek Review<a href="edit.php?post_type=trek&amp;page=reviews&amp;action=add" class="add-new-h2">Add New</a>';
		...
		<div class="wrap">
			...
			<form method="post" onsubmit="return manage_review_validateForm1();">
				...
				<table class="trav_admin_table trav_review_manage_table one-half">
					...
					<tr>
						<th><?php _e( 'Select Trek', 'trav' ); ?></th>
						<td>
							<select name="post_id" id="trek_id">
								<option></option>
								<?php
									$args = array(
											'post_type'         => 'trek',
				...
				<input type="submit" class="button-primary" name="save" value="Save Review">
				<a href="edit.php?post_type=trek&amp;page=reviews" class="button-secondary">Cancel</a>
...
if ( ! function_exists( 'trav_review_delete_action' ) ) {
	function trav_review_delete_action() {
		...
		//}
		wp_redirect( admin_url( 'edit.php?post_type=trek&page=reviews') );
		exit;
	}
}
...
if ( ! function_exists( 'trav_review_save_action' ) ) {
	function trav_review_save_action() {
		...
		if ( ! isset( $_POST['review_save'] ) || ! wp_verify_nonce( $_POST['review_save'], 'trav_review_manage' ) ) {
			...
			wp_redirect( admin_url( 'edit.php?post_type=trek&page=reviews&action=edit&review_id=' . $id . '&updated=true') );
			exit;
		}
	}
}
