...
class Trav_Acc_Booking_List_Table extends WP_List_Table {
	...
	function process_bulk_action() {
		...
		if ( 'bulk_delete'===$this->current_action() ) {
			...
			if ( current_user_can( 'manage_options' ) ) {
				$sql = sprintf('DELETE FROM %1$s WHERE id IN (%2$s)', TRAV_TREK_BOOKINGS_TABLE, "$format" );
			} else {
				$sql = sprintf('DELETE %1$s FROM %1$s INNER JOIN %2$s as trek ON trek_id=trek.ID WHERE %1$s.id IN (%3$s) AND trek.post_author = %4$d', TRAV_TREK_BOOKINGS_TABLE, $post_table_name, "$format", $current_user_id );
			}

			$wpdb->query( $wpdb->prepare( $sql, $selected_ids ) );
			wp_redirect( admin_url( 'edit.php?post_type=trek&page=bookings&bulk_delete=true') );
		}
	}
	...
	function prepare_items() {
		...
		$where = "1=1";
		if ( ! empty( $_REQUEST['trek_id'] ) ) $where .= " AND Trav_Bookings.trek_id = '" . esc_sql( trav_acc_org_id( $_REQUEST['trek_id'] ) ) . "'";
		...
		$sql = $wpdb->prepare( 'SELECT Trav_Bookings.*, trek.ID as acc_id, trek.post_title as trek_name, room_type.ID as room_type_id, room_type.post_title as room_type_name FROM %1$s as Trav_Bookings
						INNER JOIN %2$s as trek ON Trav_Bookings.trek_id=trek.ID
						INNER JOIN %2$s as room_type ON Trav_Bookings.room_type_id=room_type.ID
						WHERE ' . $where . ' ORDER BY %3$s %4$s
						LIMIT %5$s, %6$s' , TRAV_TREK_BOOKINGS_TABLE, $post_table_name, $orderby, $order, $per_page * ( $current_page - 1 ), $per_page );
		$data = $wpdb->get_results( $sql, ARRAY_A );

		$sql = sprintf( 'SELECT COUNT(*) FROM %1$s as Trav_Bookings INNER JOIN %2$s as trek ON Trav_Bookings.trek_id=trek.ID WHERE %3$s' , TRAV_TREK_BOOKINGS_TABLE, $post_table_name, $where );
...
if ( ! function_exists( 'trav_acc_booking_add_menu_items' ) ) {
	function trav_acc_booking_add_menu_items() {
		//add trek bookings list page
		$page = add_submenu_page( 'edit.php?post_type=trek', 'Trek Bookings', 'Bookings', 'edit_treks', 'bookings', 'trav_acc_booking_render_pages' );
...
if ( ! function_exists( 'trav_acc_booking_render_list_page' ) ) {
	function trav_acc_booking_render_list_page() {
		...
		<div class="wrap">
			...
			<h2>Trek Bookings <a href="edit.php?post_type=trek&amp;page=bookings&amp;action=add" class="add-new-h2">Add New</a></h2>
			...
			<select name="status" id="status_filter">
				...
				<?php
					$statuses = array( '0' => 'cancelled', '1' => 'upcoming', '2' => 'completed', /*'-1' => 'pending'*/ );
			...
			<a href="edit.php?post_type=trek&amp;page=bookings" class="button-secondary">Show All</a>
...
if ( ! function_exists( 'trav_acc_booking_render_manage_page' ) ) {
	function trav_acc_booking_render_manage_page() {
		...
		if ( 'edit' == $_REQUEST['action'] ) {
			...
			$sql = $wpdb->prepare( 'SELECT Trav_Bookings.* , trek.post_title as trek_name, room_type.post_title as room_type_name FROM %1$s as Trav_Bookings
							INNER JOIN %2$s as trek ON Trav_Bookings.trek_id=trek.ID
							INNER JOIN %2$s as room_type ON Trav_Bookings.room_type_id=room_type.ID
							WHERE ' . $where , TRAV_TREK_BOOKINGS_TABLE, $post_table_name, $booking_id );
		...
		<div class="wrap">
			<?php $page_title = ( 'edit' == $_REQUEST['action'] ) ? 'Edit Trek Booking<a href="edit.php?post_type=trek&amp;page=bookings&amp;action=add" class="add-new-h2">Add New</a>' : 'Add New Trek booking'; ?>
			...
			<form method="post" onsubmit="return manage_booking_validateForm();">
				...
				<div class="one-half">
					...
					<table class="trav_admin_table trav_booking_manage_table">
									...
									<?php
										...
										if ( $trek_query->have_posts() ) {
											while ( $trek_query->have_posts() ) {
												...
												$selected = '';
												$id = $trek_query->post->ID;
												if ( $booking_data['trek_id'] == trav_acc_org_id( $id ) ) $selected = ' selected ';
							...
							<td>
								<select name="room_type_id" id="room_type_id">
									...
									<?php
										...
										if ( ! empty( $booking_data['trek_id'] ) ) {
											$args['meta_query'] = array(
													array(
														'key'     => 'trav_room_trek',
														'value'   => trav_acc_clang_id( $booking_data['trek_id'] ),
													),
												);
										}
										if ( ! current_user_can( 'manage_options' ) ) {
						...
						<tr>
							...
							<td>
								<select name="status">
									<?php $statuses = array( '0' => 'cancelled', '1' => 'upcoming', '2' => 'completed', /*'-1' => 'pending'*/ );
				...
				<a href="edit.php?post_type=trek&amp;page=bookings" class="button-secondary">Cancel</a>
...
if ( ! function_exists( 'trav_acc_booking_delete_action' ) ) {
	function trav_acc_booking_delete_action() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			$sql = $wpdb->prepare( 'SELECT Trav_Bookings.trek_id FROM %1$s as Trav_Bookings WHERE Trav_Bookings.id = %2$d' , TRAV_TREK_BOOKINGS_TABLE, $_REQUEST['booking_id'] );
		...
		// do action
		$wpdb->delete( TRAV_TREK_BOOKINGS_TABLE, array( 'id' => $_REQUEST['booking_id'] ) );
		wp_redirect( admin_url( 'edit.php?post_type=trek&page=bookings') );
		exit;
	}
}
		...
		if ( ! isset( $_POST['booking_save'] ) || ! wp_verify_nonce( $_POST['booking_save'], 'trav_manage_bookings' ) ) {
		   ...
		   exit;
			...
			if ( empty( $_POST['id'] ) ) {
				...
				$wpdb->insert( TRAV_TREK_BOOKINGS_TABLE, $data );
				$id = $wpdb->insert_id;
			} else {
				//update
				$wpdb->update( TRAV_TREK_BOOKINGS_TABLE, $data, array( 'id' => sanitize_text_field( $_POST['id'] ) ) );
				$id = sanitize_text_field( $_POST['id'] );
			}
			wp_redirect( admin_url( 'edit.php?post_type=trek&page=bookings&action=edit&booking_id=' . $id . '&updated=true') );
			exit;
		}
	}
}
