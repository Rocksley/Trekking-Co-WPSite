...
/*
 * Check if a given trek is available for a given day and return vacany number
 */
if ( ! function_exists( 'trav_acc_get_day_vacancy' ) ) {
	function trav_acc_get_day_vacancy( $acc_id, $date, $room_type_id = '' ) {
		//SELECT (vacancies.rooms - IFNULL(bookings.rooms,0)) AS rooms FROM (SELECT trek_id, SUM(rooms) AS rooms FROM {$wpdb->prefix}trav_trek_vacancies WHERE 1=1 AND trek_id='39' AND date_from <= '2014-06-24'  AND date_to > '2014-06-24') AS vacancies, (SELECT trek_id, SUM(rooms) AS rooms FROM {$wpdb->prefix}trav_trek_bookings WHERE 1=1 AND trek_id='39' AND date_from <= '2014-06-24'  AND date_to > '2014-06-24') AS bookings
		global $wpdb;
		$acc_id = esc_sql( trav_acc_org_id( $acc_id ) );
		$room_type_id = esc_sql( trav_room_org_id( $room_type_id ) );
		$date = esc_sql( $date );
		$where = '1=1';
		if ( ! empty( $acc_id ) ) $where .= " AND trek_id='{$acc_id}'";
		...
		FROM (SELECT SUM(rooms) AS rooms FROM " . TRAV_TREK_VACANCIES_TABLE . " WHERE {$where}) AS vacancies, 
		(SELECT SUM(rooms) AS rooms FROM " . TRAV_TREK_BOOKINGS_TABLE . " WHERE {$where} AND status!='0' ) AS bookings";
...
/*
 * Check if a given trek is available for a given month and return vacancies array
 */
...
if ( ! function_exists( 'trav_acc_get_available_rooms' ) ) {
	function trav_acc_get_available_rooms( $acc_id, $from_date, $to_date, $rooms=1, $adults=1, $kids, $child_ages, $except_booking_no=0, $pin_code=0 ) {
		...
		$minimum_stay = get_post_meta( $acc_id, 'trav_trek_minimum_stay', true );
		...
		$sql = "SELECT DISTINCT pm0.post_id FROM " . $wpdb->postmeta . " as pm0 INNER JOIN " . $wpdb->posts . " AS room ON (pm0.post_id = room.ID) AND (room.post_status = 'publish') AND (room.post_type = 'room_type') WHERE meta_key = 'trav_room_trek' AND meta_value = " . esc_sql( $acc_id );
		...
		// get available trek room_type_id based on max_adults and max_kids
		$sql = "SELECT DISTINCT pm0.post_id AS room_type_id FROM (SELECT * FROM " . $wpdb->postmeta . " WHERE meta_key = 'trav_room_trek' AND meta_value = " . esc_sql( $acc_id ) . " ) AS pm0 
		...
		// get available trek room_type_id and price based on date
		// initiate variables
		$check_dates = array();
		$price_data = array();
		...
		foreach ( $period as $dt ) {
			...
			$sql = "SELECT vacancies.room_type_id, vacancies.price_per_room , vacancies.price_per_person, vacancies.child_price
					FROM (SELECT room_type_id, rooms, price_per_room, price_per_person, child_price
							FROM " . TRAV_TREK_VACANCIES_TABLE . " 
							WHERE 1=1 AND trek_id='" . $acc_id . "' AND room_type_id IN (" . implode( ',', $bookable_room_ids ) . ") AND date_from <= '" . $check_date . "'  AND date_to > '" . $check_date . "' ) AS vacancies
					LEFT JOIN (SELECT room_type_id, SUM(rooms) AS rooms 
							FROM " . TRAV_TREK_BOOKINGS_TABLE . " 
							WHERE 1=1 AND status!='0' AND trek_id='" . $acc_id . "' AND date_to > '" . $check_date . "'  AND date_from <= '" . $check_date . "'" . ( ( empty( $except_booking_no ) || empty( $pin_code ) )?"":( " AND NOT ( booking_no = '" . $except_booking_no . "' AND pin_code = '" . $pin_code . "' )" ) ) . " GROUP BY room_type_id
...
/*
 * Calculate the price of selected trek room and return price array data
 */
if ( ! function_exists( 'trav_acc_get_room_price_data' ) ) {
	function trav_acc_get_room_price_data( $acc_id, $room_type_id, $from_date, $to_date, $rooms=1, $adults=1, $kids=0, $child_ages, $except_booking_no=0, $pin_code=0 ) {
		...
		//validation
		if ( ! is_array( $child_ages ) ){$child_ages = unserialize($child_ages);}
		$room_trek_id = get_post_meta( $room_type_id, 'trav_room_trek', true );
		if ( $room_trek_id != $acc_id ) return false;
		...
		foreach ( $period as $dt ) {
			...
			$sql = "SELECT vacancies.room_type_id, vacancies.price_per_room , vacancies.price_per_person, vacancies.child_price
					FROM (SELECT room_type_id, rooms, price_per_room, price_per_person, child_price
							FROM " . TRAV_TREK_VACANCIES_TABLE . " 
							WHERE 1=1 AND trek_id='" . $acc_id . "' AND room_type_id = '" . $room_type_id . "' AND date_from <= '" . $check_date . "'  AND date_to > '" . $check_date . "' ) AS vacancies
					LEFT JOIN (SELECT room_type_id, SUM(rooms) AS rooms 
							FROM " . TRAV_TREK_BOOKINGS_TABLE . " 
							WHERE 1=1 AND status!='0' AND trek_id='" . $acc_id . "' AND room_type_id = '" . $room_type_id . "' AND date_to > '" . $check_date . "' AND date_from <= '" . $check_date . "'" . ( ( empty( $except_booking_no ) || empty( $pin_code ) )?"":( " AND NOT ( booking_no = '" . $except_booking_no . "' AND pin_code = '" . $pin_code . "' )" ) ) . "
					) AS bookings ON vacancies.room_type_id = bookings.room_type_id
					WHERE vacancies.rooms - IFNULL(bookings.rooms,0) >= " . $rooms . ";";
...
/*
 * Get special( latest or featured ) treks and return data
 */
if ( ! function_exists( 'trav_acc_get_special_accs' ) ) {
	function trav_acc_get_special_accs( $type='latest', $count=10, $exclude_ids=array(),  $country='', $city='', $acc_type=array() ) {
		...
		if ( $type == 'featured'  ) {
			...
			$sql = 'SELECT trek_id, COUNT(*) AS booking_count FROM ' . TRAV_TREK_BOOKINGS_TABLE . ' AS booking';
			$where = ' WHERE (booking.status <> 0) AND (booking.created > %s)';
			if ( ! empty( $country ) ) {
				$sql .= " INNER JOIN {$tbl_postmeta} AS meta_c1 ON (meta_c1.meta_key = 'trav_trek_country') AND (booking.trek_id = meta_c1.post_id)";
				$where .= " AND meta_c1.meta_value like '%%{$country}%%'";
			}
			if ( ! empty( $city ) ) {
				$sql .= " INNER JOIN {$tbl_postmeta} AS meta_c1 ON (meta_c1.meta_key = 'trav_trek_city') AND (booking.trek_id = meta_c1.post_id)";
			...
			if ( ! empty( $acc_type ) ) {
				...
				$where .= " AND tt.taxonomy = 'trek_type' AND tt.term_id IN (" . esc_sql( implode( ',', $acc_type ) ) . ")";
			}
			$sql .= $where . ' GROUP BY booking.trek_id ORDER BY booking_count desc LIMIT %d';
			$popular_accs = $wpdb->get_results( $wpdb->prepare( $sql, $date, $count ) );
			$result = array();
			if ( ! empty( $popular_accs ) ) {
				foreach ( $popular_accs as $acc ) {
					$result[] = get_post( trav_acc_clang_id( $acc->trek_id ) );
				}
			}
			// if booked room number in last month is smaller than count then add latest accs
			if ( count( $popular_accs ) < $count ) {
				foreach ( $popular_accs as $acc ) {
					$exclude_ids[] = trav_acc_clang_id( $acc->trek_id );
				}
...
/*
 * Get discounted(hot) treks and return data
 */
	...
	function trav_acc_get_accs_from_id( $ids ) {
		...
		foreach( $ids as $id ) {
			...
			if ( ! empty( $result ) && ! is_wp_error( $result ) ) {
				if ( $result->post_type == 'trek' ) $results[] = $result;
			}
		}
		return $results;
	}
...
if ( ! function_exists( 'trav_acc_get_city' ) ) {
	function trav_acc_get_city( $acc_id ) {
		return trav_post_get_location( $acc_id, 'trek', 'city' );
	}
}

/*
 * Get Trek Country Data
 */
if ( ! function_exists( 'trav_acc_get_country' ) ) {
	function trav_acc_get_country( $acc_id ) {
		return trav_post_get_location( $acc_id, 'trek', 'country' );
	}
}

/*
 * Get Trek Search Result
 */
	...
	function trav_acc_get_search_result( $s='', $date_from='', $date_to='', $rooms=1, $adults=1, $kids=0, $order_by='acc_title', $order='ASC', $last_no=0, $per_page=12, $min_price=0, $max_price='no_max', $rating=0, $acc_type, $amenities ) {
		...
		if ( ! empty( $s ) ) {
			$s_query = "SELECT DISTINCT post_s1.ID AS acc_id FROM {$tbl_posts} AS post_s1 
						...
						LEFT JOIN {$tbl_terms} AS terms_s1 ON (meta_s1.meta_key IN('trav_trek_country','trav_trek_city')) AND (terms_s1.term_id = meta_s1.meta_value)
						WHERE (post_s1.post_status = 'publish') AND (post_s1.post_type = 'trek')
		...
		if ( defined('ICL_LANGUAGE_CODE') && ( $language_count > 1 ) ) {
			$s_query = "SELECT DISTINCT it2.element_id AS acc_id FROM ({$s_query}) AS t0
						INNER JOIN {$tbl_icl_translations} it1 ON (it1.element_type = 'post_trek') AND it1.element_id = t0.acc_id
						INNER JOIN {$tbl_icl_translations} it2 ON (it2.element_type = 'post_trek') AND it2.language_code='" . trav_get_default_language() . "' AND it2.trid = it1.trid ";
		}
		...
		$c_query = "SELECT t1.*, meta_c1.post_id AS room_id, meta_c2.meta_value AS max_adults, meta_c3.meta_value AS max_kids, meta_c4.meta_value AS minimum_stay
					FROM ( {$s_query} ) AS t1
					INNER JOIN {$tbl_postmeta} AS meta_c1 ON (meta_c1.meta_key = 'trav_room_trek') AND (t1.acc_id = meta_c1.meta_value)
					...
					LEFT JOIN {$tbl_postmeta} AS meta_c4 ON (t1.acc_id = meta_c4.post_id) AND (meta_c4.meta_key='trav_trek_minimum_stay')";
		...
		if ( $from_date_obj && $to_date_obj ) {
			...
			$v_query = "SELECT t3.acc_id, t3.room_id, t3.max_adults, t3.max_kids, t3.minimum_stay, MIN(rooms) AS min_rooms FROM (
							...
							LEFT JOIN " . TRAV_TREK_VACANCIES_TABLE . " AS vacancies ON (vacancies.room_type_id = t2.room_id) AND (vacancies.date_from <= check_dates.check_date AND vacancies.date_to > check_dates.check_date)
							LEFT JOIN " . TRAV_TREK_BOOKINGS_TABLE . " AS bookings ON bookings.status!='0' AND (bookings.room_type_id = t2.room_id) AND (bookings.date_from <= check_dates.check_date AND bookings.date_to > check_dates.check_date)
		...
		if ( defined('ICL_LANGUAGE_CODE') && ( $language_count > 1 ) && ( trav_get_default_language() != ICL_LANGUAGE_CODE ) ) {
			$sql = "SELECT it4.element_id AS acc_id FROM ({$sql}) AS t5
					INNER JOIN {$tbl_icl_translations} it3 ON (it3.element_type = 'post_trek') AND it3.element_id = t5.acc_id
					INNER JOIN {$tbl_icl_translations} it4 ON (it4.element_type = 'post_trek') AND it4.language_code='" . ICL_LANGUAGE_CODE . "' AND it4.trid = it3.trid";
		}
		...
		$sql = "SELECT t1.*, post_l1.post_title as acc_title, meta_rating.meta_value as review, meta_price.meta_value as avg_price FROM {$temp_tbl_name} as t1
				INNER JOIN {$tbl_posts} post_l1 ON (t1.acc_id = post_l1.ID) AND (post_l1.post_status = 'publish') AND (post_l1.post_type = 'trek')
				LEFT JOIN {$tbl_postmeta} AS meta_rating ON (t1.acc_id = meta_rating.post_id) AND (meta_rating.meta_key = 'review')
				LEFT JOIN {$tbl_postmeta} AS meta_price ON (t1.acc_id = meta_price.post_id) AND (meta_price.meta_key = 'trav_trek_avg_price')";
		$where = ' 1=1';
...
if ( ! function_exists( 'trav_acc_get_search_result_count' ) ) {
	function trav_acc_get_search_result_count( $min_price, $max_price, $rating, $acc_type, $amenities ) {
		...
		if ( $min_price != 0 || $max_price != 'no_max' ) {
			$sql .= " INNER JOIN {$tbl_postmeta} AS meta_price ON (t1.acc_id = meta_price.post_id) AND (meta_price.meta_key = 'trav_trek_avg_price')";
		}
		if ( $min_price != 0 ) {
			$where .= " AND cast(meta_price.meta_value as unsigned) >= {$min_price}";
		}
...
if ( ! function_exists( 'trav_acc_get_similar' ) ) {
	function trav_acc_get_similar( $acc_id, $limit ) {
		global $wpdb, $language_count;
		$tbl_icl_translations = esc_sql( $wpdb->prefix . 'icl_translations' );
		$city = esc_sql( get_post_meta( $acc_id, 'trav_trek_city', true ) );
		$country = esc_sql( get_post_meta( $acc_id, 'trav_trek_country', true ) );
		$star_rating = esc_sql( get_post_meta( $acc_id, 'trav_trek_star_rating', true ) );

		$avg_price = esc_sql( get_post_meta( $acc_id, 'trav_trek_avg_price', true ) );
		$avg_price = empty($avg_price) ? 0 : esc_sql( $avg_price );

		$trek_type = wp_get_post_terms( $acc_id, 'trek_type' );
		$acc_type_id = ( ! empty( $trek_type ) ) ? esc_sql( $trek_type[0]->term_id ) : 0;
		$acc_id = esc_sql( $acc_id );
		$limit = esc_sql( $limit );
		...
		$sql = "SELECT DISTINCT post1.ID, ( IF( IFNULL(meta1.meta_value,'')='{$city}', 8, 0) + IF( IFNULL(meta2.meta_value,'')='{$country}', 4, 0) + IF( IFNULL(meta3.meta_value,'')='{$star_rating}', 1, 0) + IF( IFNULL(tt.term_id,0)={$acc_type_id}, 2, 0) ) AS similarity, ABS( IFNULL(meta4.meta_value,0) - {$avg_price} ) as price_dist
				...
				LEFT JOIN {$tbl_postmeta}  AS meta1 ON meta1.meta_key = 'trav_trek_city' AND post1.ID = meta1.post_id
				LEFT JOIN {$tbl_postmeta}  AS meta2 ON meta2.meta_key = 'trav_trek_country' AND post1.ID = meta2.post_id
				LEFT JOIN {$tbl_postmeta}  AS meta3 ON meta3.meta_key = 'trav_trek_star_rating' AND post1.ID = meta3.post_id
				LEFT JOIN {$tbl_postmeta}  AS meta4 ON meta4.meta_key = 'trav_trek_avg_price' AND post1.ID = meta4.post_id
				LEFT JOIN {$tbl_term_relationships} AS tr ON post1.ID = tr.object_id
				INNER JOIN {$tbl_term_taxonomy} AS tt ON tt.taxonomy = 'trek_type' AND tr.term_taxonomy_id = tt.term_taxonomy_id";

		// if wpml is enabled do search by default language post
		if ( defined('ICL_LANGUAGE_CODE') && ( $language_count > 1 ) ) {
			$sql .= " INNER JOIN {$tbl_icl_translations} AS it2 ON it2.element_type = 'post_trek' AND it2.language_code='" . ICL_LANGUAGE_CODE . "' AND it2.element_id = post1.ID";
		}
		$sql .= " WHERE post_type = 'trek' AND post_status='publish' AND post1.ID != {$acc_id} ORDER BY similarity DESC, price_dist ASC LIMIT {$limit}";
...
if ( ! function_exists( 'trav_acc_update_booking' ) ) {
	function trav_acc_update_booking( $booking_no, $pin_code, $new_data, $action='update' ) {
		...
		$result = $wpdb->update( TRAV_TREK_BOOKINGS_TABLE, $new_data, array( 'booking_no' => $booking_no, 'pin_code' => $pin_code ) );
		if ( $result ) {
...
if ( ! function_exists( 'trav_acc_get_star_rating' ) ) {
	function trav_acc_get_star_rating( $acc_id ) {
		$trek_type = wp_get_post_terms( $acc_id, 'trek_type' );
		$hotel_star_rating = get_post_meta( $acc_id, 'trav_trek_star_rating', true );
		$acc_type_name = '';
		$result = '';
		if ( ! empty ( $trek_type ) ) {
			$acc_type_name =  $trek_type[0]->name;
		}
...
if ( ! function_exists( 'trav_acc_send_confirmation_email' ) ) {
	function trav_acc_send_confirmation_email( $booking_no, $booking_pincode, $type='new', $subject='', $description='' ) {
		...
		if ( ! empty( $booking_data ) ) {
			...
			$booking_data['trek_id'] = trav_acc_clang_id( $booking_data['trek_id'] );
			$booking_data['room_type_id'] = trav_room_clang_id( $booking_data['room_type_id'] );

			// trek info
			$trek_name = get_the_title( $booking_data['trek_id'] );
			$trek_url = esc_url( trav_get_permalink_clang( $booking_data['trek_id'] ) );
			$trek_thumbnail = get_the_post_thumbnail( $booking_data['trek_id'], 'list-thumb' );
			$trek_address = get_post_meta( $booking_data['trek_id'], 'trav_trek_address', true );
			$trek_city = trav_acc_get_city($booking_data['trek_id']);
			$trek_country = trav_acc_get_country($booking_data['trek_id']);
			$trek_phone = get_post_meta( $booking_data['trek_id'], 'trav_trek_phone', true );
			$trek_email = get_post_meta( $booking_data['trek_id'], 'trav_trek_email', true );
			$trek_room_name = esc_html( get_the_title( $booking_data['room_type_id'] ) );
			if ( empty( $trek_address ) ) {
				$trek_address = $trek_city . ' ' . $trek_country;
			}

			$check_in = get_post_meta( $booking_data['trek_id'], 'trav_trek_check_in', true );
			$check_out = get_post_meta( $booking_data['trek_id'], 'trav_trek_check_out', true );
			...
			if ( ! empty( $trav_options['acc_confirm_email_ical'] ) && ( $type == 'new' ) ) {
				$mail_sent = trav_send_ical_event( $site_name, $admin_email, $customer_first_name . ' ' . $customer_last_name, $customer_email, $check_in_time, $check_out_time, $subject, $description, $trek_address);
			} else {
			...
			if ( ! empty( $trav_options['acc_booked_notify_bowner'] ) ) {
				...
				if ( ! empty( $trek_email ) ) {
					$bowner_address = $trek_email;
				} else {
					$post_author_id = get_post_field( 'post_author', $booking_data['trek_id'] );
...
if ( ! function_exists( 'trav_acc_conf_send_mail' ) ) {
	function trav_acc_conf_send_mail( $booking_data ) {
		...
		if ( trav_acc_send_confirmation_email( $booking_data['booking_no'], $booking_data['pin_code'], 'new' ) ) {
			$mail_sent = 1;
			$wpdb->update( TRAV_TREK_BOOKINGS_TABLE, array( 'mail_sent' => $mail_sent ), array( 'booking_no' => $booking_data['booking_no'], 'pin_code' => $booking_data['pin_code'] ), array( '%d' ), array( '%d','%d' ) );
		}
	}
}
...
if ( ! function_exists( 'trav_acc_get_booking_data' ) ) {
	function trav_acc_get_booking_data( $booking_no, $pin_code ) {
		global $wpdb;
		return $wpdb->get_row( 'SELECT * FROM ' . TRAV_TREK_BOOKINGS_TABLE . ' WHERE booking_no="' . esc_sql( $booking_no ) . '" AND pin_code="' . esc_sql( $pin_code ) . '"', ARRAY_A );
	}
}
...
/*
 * generate js variable data to transfer trek.js
 */
if ( ! function_exists( 'trav_get_acc_js_data' ) ) {
	function trav_get_acc_js_data() {
		global $post, $trav_options;
		$loc = get_post_meta( $post->ID, "trav_trek_loc" );
		$main_top_meta = get_post_meta( $post->ID, "trav_trek_main_top" );
		...
		$minimum_stay = get_post_meta( $post->ID, 'trav_trek_minimum_stay', true );
		...
		//messages
		$acc_data['msg_no_booking_page'] = __( 'Please set trek booking page on admin/Theme Options/Page Settings', 'trav' );
		...
		$acc_data['msg_wrong_date_5'] = sprintf( __( 'Minimum stay for this trek is %d nights. Have another look at your dates and try again.', 'trav' ), $minimum_stay );
...
/*
 * trek booking page before action
 */
if ( ! function_exists( 'trav_acc_booking_before' ) ) {
	function trav_acc_booking_before() {
		...
		// init booking data : array( 'trek_id', 'room_type_id', 'date_from', 'date_to', 'rooms', 'adults', 'kids', 'child_ages' );
		$raw_booking_data = '';
		parse_str( $_REQUEST['booking_data'], $raw_booking_data );

		//verify nonce
		if ( ! isset( $raw_booking_data['_wpnonce'] ) || ! wp_verify_nonce( $raw_booking_data['_wpnonce'], 'post-' . $raw_booking_data['trek_id'] ) ) {
			do_action('trav_acc_booking_wrong_data');
			exit;
		}

		// init booking_data fields
		$booking_fields = array( 'trek_id', 'room_type_id', 'date_from', 'date_to', 'rooms', 'adults', 'kids', 'child_ages' );
		$booking_data = array();
		...
		// get price data
		$room_price_data = trav_acc_get_room_price_data( $booking_data['trek_id'], $booking_data['room_type_id'], $booking_data['date_from'], $booking_data['date_to'], $booking_data['rooms'], $booking_data['adults'], $booking_data['kids'], $booking_data['child_ages'] );
		$acc_url = get_permalink( $booking_data['trek_id'] );
		...
		$tax_rate = get_post_meta( $booking_data['trek_id'], 'trav_trek_tax_rate', true );
		$tax = 0;
		...
		$deposit_rate = get_post_meta( $booking_data['trek_id'], 'trav_trek_security_deposit', true );
		...
		$_SESSION['booking_data'][$transaction_id] = $booking_data; //'trek_id', 'room_type_id', 'date_from', 'date_to', 'rooms', 'adults', 'kids', 'child_ages', room_price, tax, total_price, currency_code, exchange_rate, deposit_price

		$review = get_post_meta( trav_acc_org_id( $booking_data['trek_id'] ), 'review', true );
...
/*
 * get acc list belongs to travel package
 */
if ( ! function_exists( 'trav_acc_get_accs_by_tg_id' ) ) {
	function trav_acc_get_accs_by_tg_id( $tg_id, $order_by='name', $order='ASC', $last_no=0, $per_page=12 ) {
		...
		if ( $order_by == 'name' ) {
			...
			$args['meta_key'] = 'trav_trek_avg_price';
