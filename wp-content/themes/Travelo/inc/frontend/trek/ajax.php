...
/*
 * Handle get trek month vacancies ajax request.
 */
if ( ! function_exists( 'trav_ajax_acc_get_month_vacancies' ) ) {
	function trav_ajax_acc_get_month_vacancies() {
		...
		//validation
		if ( ! isset( $_POST['year'] ) || ! isset( $_POST['month'] ) || ! isset( $_POST['trek_id'] ) ) {
		...
		//initiate variables
		$acc_id = sanitize_text_field( $_POST['trek_id'] );
...
if ( ! function_exists( 'trav_ajax_acc_get_available_rooms' ) ) {
	function trav_ajax_acc_get_available_rooms() {
		...
		$result_json = array( 'success' => 0, 'result' => '' );
		if ( ! isset( $_POST['_wpnonce'] ) || ! isset( $_POST['trek_id'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'post-' . sanitize_text_field( $_POST['trek_id'] ) ) ) {
			$result_json['success'] = 0;
		...
		if ( isset( $_POST['trek_id'] ) && isset( $_POST['date_from'] ) && trav_strtotime( $_POST['date_from'] ) && isset( $_POST['date_to'] ) && trav_strtotime( $_POST['date_to'] ) && ( ( time()-(60*60*24) ) < trav_strtotime( $_POST['date_from'] ) ) ) {
			$acc_id = (int) $_POST['trek_id'];
			$except_booking_no = 0; $pin_code = 0;
...
if ( ! function_exists( 'trav_ajax_acc_submit_review' ) ) {
	function trav_ajax_acc_submit_review() {
		...
		if ( ! is_array( $booking_data ) || $booking_data['status'] == 0 || trav_acc_org_id($data['post_id']) != $booking_data['trek_id'] ) {
			$result_json['success'] = 0;
		...
		$data['post_id'] = $booking_data['trek_id'];
		...
		if ( ! $review_data = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . TRAV_REVIEWS_TABLE . ' WHERE booking_no=%d AND pin_code=%d', $data['booking_no'], $data['pin_code'] ), ARRAY_A ) ) {
...
if ( ! function_exists( 'trav_ajax_acc_add_to_wishlist' ) ) {
	function trav_ajax_acc_add_to_wishlist() {
		...
		$new_item_id = sanitize_text_field( trav_acc_org_id( $_POST['trek_id'] ) );
		...
		if ( isset( $_POST['remove'] ) ) {
			...
			if ( update_user_meta( $user_id, 'wishlist', $wishlist ) ) {
				$result_json['success'] = 1;
				$result_json['result'] = __( 'This trek has removed from your wishlist successfully.', 'trav' );
			} else {
				$result_json['success'] = 0;
			...
			if ( ! in_array( $new_item_id, $wishlist) ) {
				...
				if ( update_user_meta( $user_id, 'wishlist', $wishlist ) ) {
					$result_json['success'] = 1;
					$result_json['result'] = __( 'This trek has added to your wishlist successfully.', 'trav' );
				} else {
					$result_json['success'] = 0;
					$result_json['result'] = __( 'Sorry, An error occurred while update wishlist.', 'trav' );
				}
...
if ( ! function_exists( 'trav_ajax_acc_submit_booking' ) ) {
	function trav_ajax_acc_submit_booking() {
		...
		$booking_fields = array( 'trek_id', 'room_type_id', 'rooms', 'adults', 'kids', 'child_ages', 'total_price', 'room_price', 'tax', 'currency_code', 'exchange_rate', 'deposit_price', 'date_from', 'date_to', 'created', 'booking_no', 'pin_code', 'status' );
		$booking_data = array();
		...
		$latest_booking_id = $wpdb->get_var( 'SELECT id FROM ' . TRAV_TREK_BOOKINGS_TABLE . ' ORDER BY id DESC LIMIT 1' );
		...
		$room_price_data = trav_acc_get_room_price_data( $data['trek_id'], $data['room_type_id'], $booking_data['date_from'], $booking_data['date_to'], $data['rooms'], $data['adults'], $data['kids'], $data['child_ages'] );
...
if ( ! function_exists( 'trav_ajax_acc_check_room_availability' ) ) {
	function trav_ajax_acc_check_room_availability() {
		...
		$room_price_data = trav_acc_get_room_price_data( $booking_data['trek_id'], $booking_data['room_type_id'], $booking_data['date_from'], $booking_data['date_to'], $booking_data['rooms'], $booking_data['adults'], $booking_data['kids'], $booking_data['child_ages'], $booking_no, $pin_code );
