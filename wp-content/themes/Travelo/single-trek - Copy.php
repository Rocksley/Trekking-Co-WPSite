...
if ( have_posts() ) {
	while ( have_posts() ) : the_post();
		...
		$tm_data = get_post_meta( $acc_id, 'trav_trek_tm_testimonial', true );
...
//					'value' => array( $acc_id )
		...
		$things_to_do = empty( $acc_meta['trav_trek_ttd'] ) ? '' : $acc_meta['trav_trek_ttd'];
		...
		$map = empty( $acc_meta['trav_trek_loc'] ) ? '' : $acc_meta['trav_trek_loc'][0];
		$calendar_desc = empty( $acc_meta['trav_trek_calendar_txt'] ) ? '' : $acc_meta['trav_trek_calendar_txt'][0];
		$show_gallery = 0;
		$show_map = 0;
		$show_street_view = 0;
		$show_calendar = 0;
		if ( array_key_exists( 'trav_trek_main_top', $acc_meta ) ) {
			$main_top_meta = $acc_meta['trav_trek_main_top'];
		...
		<section id="content">
			<div class="container">
				<div class="row">
					<div id="main" class="col-sm-8 col-md-9">
						<div class="tab-container style1" id="hotel-main-content">
							<ul class="tabs">
								...
								<?php if ( ! empty( $acc_meta['trav_trek_tg'] ) ) { ?>
									<li class="pull-right"><a class="button btn-small yellow-bg white-color" href="<?php echo esc_url( get_permalink( $acc_meta['trav_trek_tg'][0] ) ); ?>"><?php _e( 'TRAVEL PACKAGE', 'trav' ) ?></a></li>
								<?php } ?>
						...
						<div id="hotel-features" class="tab-container">
							<ul class="tabs">
								<?php $def_tab = ( ! empty(  $acc_meta['trav_trek_def_tab'] ) ) ? $acc_meta['trav_trek_def_tab'][0] : 'desc';?>
								...
								<?php if ( ! empty( $acc_meta['trav_trek_faq'] ) ) : ?>
									<li><a href="#hotel-faqs" data-toggle="tab"><?php _e( 'FAQs','trav' ); ?></a></li>
								<?php endif ?>
								<?php if ( ! empty( $things_to_do ) ) : ?>
									<li><a href="#hotel-things-todo" data-toggle="tab"><?php _e( 'Things to Do','trav' ); ?></a></li>
								<?php endif; ?>
                                <!-- Here Rob
								<li><a href="#hotel-write-review" data-toggle="tab"><?php // _e( 'Write a Review','trav' ); ?></a></li><br />
								-->
							</ul>
							<div class="tab-content">
								<div class="tab-pane fade<?php echo ( $def_tab == 'rooms' ) ? ' in active' : '' ?>" id="hotel-availability">
									<form id="check_availability_form" method="post">
										<input type="hidden" name="trek_id" value="<?php echo esc_attr( $acc_id ); ?>">
									...
									<div class="intro table-wrapper full-width hidden-table-sms">
										<div class="col-sm-4 features table-cell">
											...
											<?php
												...
												//trek type
												if ( ! empty ( $trek_type ) ) {
													echo sprintf( $tr, __( 'Type', 'trav' ), esc_attr( $trek_type[0]->name ) );
												}
												...
												foreach ( $detail_fields as $field => $value ) {
													if ( empty( $$field ) ) $$field = empty( $acc_meta["trav_trek_$field"] )?'':$acc_meta["trav_trek_$field"][0];
													if ( ! empty( $$field ) ) {
										...
										<?php
											if ( ! empty( $tm_data ) ) {
												$tm_style = empty( $acc_meta['trav_trek_tm_style'] )?'':$acc_meta['trav_trek_tm_style'][0];
												$tm_title = empty( $acc_meta['trav_trek_tm_title'] )?'':$acc_meta['trav_trek_tm_title'][0];
												$tm_author_photo_size = empty( $acc_meta['trav_trek_tm_author_photo_size'] )?'':$acc_meta['trav_trek_tm_author_photo_size'][0];
												$tm_class = empty( $acc_meta['trav_trek_tm_class'] )?'':$acc_meta['trav_trek_tm_class'][0];
									...
									<div class="long-description">
										...
										<div class="box policies-box">
											...
											<?php
												...
												$detail_fields = array( 'check_in' => array( 'label' => __('Check-in', 'trav'), 'pre' => '', 'sur' => '' ),
																		'check_out' => array( 'label' => __('Check-out', 'trav'), 'pre' => '', 'sur' => '' ),
												...
												foreach ( $detail_fields as $field => $value ) {
													$$field = empty( $acc_meta["trav_trek_$field"] )?'':$acc_meta["trav_trek_$field"][0];
													if ( ! empty( $$field ) ) {
														$content = $value['pre'] . $$field . $value['sur'];
														echo sprintf( $tr, esc_html( $value['label'] ), esc_html( $content ) );
													}
								...
								<div class="tab-pane fade<?php echo ( $def_tab == 'amenity' ) ? ' in active' : '' ?>" id="hotel-amenities">
									...
									<p>
										<?php
											echo esc_attr( empty( $acc_meta["trav_trek_other_amenity_info"] ) ? '' : $acc_meta["trav_trek_other_amenity_info"][0] );
										?>
									</p>
								...
								<?php if ( ! empty( $acc_meta['trav_trek_faq'] ) ) : ?>
									<div class="tab-pane fade" id="hotel-faqs">
										<?php echo do_shortcode( $acc_meta['trav_trek_faq'][0] ); ?>
									</div>
								<?php endif; ?>
								<?php if ( ! empty( $things_to_do ) ) : ?>
									<div class="tab-pane fade" id="hotel-things-todo">
										<h2><?php _e('Things to Do', 'trav');?></h2>
										<p><?php echo esc_html( empty( $acc_meta['trav_trek_ttd_detail'] )?'':$acc_meta['trav_trek_ttd_detail'][0] ); ?></p>
								...
								<div class="tab-pane fade" id="hotel-write-review">
									<?php
										...
										if ( is_user_logged_in() ) {
											$booking_data = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . TRAV_TREK_BOOKINGS_TABLE . ' WHERE trek_id=%d AND user_id=%d AND date_to<%s ORDER BY date_to DESC', trav_acc_org_id( $acc_id ), get_current_user_id(), date("Y-m-d") ), ARRAY_A );
											if ( ! empty( $booking_data ) ) {
												$review_data = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . TRAV_REVIEWS_TABLE . ' WHERE booking_no=%d AND pin_code=%d', $booking_data['booking_no'], $booking_data['pin_code'] ), ARRAY_A );
                    ...
                    <!-- Here Rob // Section removed See single-trek Copy.php for original version
					<div class="sidebar col-sm-4 col-md-3">
						<article class="detailed-logo">
							<?php if ( isset( $acc_meta['trav_trek_logo'] ) ) { ?>
								<figure>
									<img width="114" src="<?php echo esc_url( wp_get_attachment_url( $acc_meta['trav_trek_logo'][0] ) );?>" alt="Trek Logo">
								</figure>
							<?php } ?>
							<div class="details">
								<h2 class="box-title">
								...
								<?php if ( isset( $acc_meta['trav_trek_avg_price'] ) && is_numeric( $acc_meta['trav_trek_avg_price'][0] ) ) { ?>
									<span class="price clearfix">
										<small class="pull-left"><?php _e( 'avg/night', 'trav' ); ?></small>
										<span class="pull-right"><?php echo esc_html( trav_get_price_field( $acc_meta['trav_trek_avg_price'][0] ) ); ?></span>
									</span>
								<?php } ?>
								...
								<p class="description">
									<?php
										if ( isset( $acc_meta['trav_trek_brief'] ) ) {
											echo esc_html( $acc_meta['trav_trek_brief'][0] );
										} else {
