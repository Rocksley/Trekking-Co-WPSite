<?php
/*
 * Plugin Name: Responsive Styled Google Maps (shared on wplocker.com)
 ...
 */
function resmap_css() {
    wp_register_style('resmap_css', plugins_url('includes/css/resmap.min.css', __FILE__), false, '2.29');
...
function resmap_scripts() {
    ...
    if (is_ssl()) {
            wp_register_script('googlemapsapi', 
                              'https://maps-api-ssl.google.com/maps/api/js?v=3.exp&libraries=adsense,places',
			// Here Rob	'https://maps.googleapis.com/maps/api/js?v=3.27&amp;key=AIzaSyDTfbNcaNXBfrI4XrobnI_aXxBCTLRIcpI',
                               array('jquery'), 
                               null, 
                               true);
    } else {
            wp_register_script('googlemapsapi', 
	               //  Here Rob	       'http://maps.googleapis.com/maps/api/js?v=3.exp&libraries=adsense,places', 
			      'https://maps.googleapis.com/maps/api/js?v=3.27&amp;key=AIzaSyDTfbNcaNXBfrI4XrobnI_aXxBCTLRIcpI',
                              array('jquery'), 
                              null, 
                              true);
    }
    ...
    // Then, we enqueue scripts only when the shortcode function is called, to avoid unneccessary loading in all pages.
}
...
function resmap_admin_css($hook) {
    ...
    wp_register_style('resmap_css', plugins_url('includes/css/resmap.min.css', __FILE__), false, '2.29');
    wp_register_style('resmap_admin_css', plugins_url('includes/css/resmap.admin.min.css', __FILE__), false, '2.29');
    wp_enqueue_style('resmap_css');
...
function resmap_admin_scripts($hook) {
    ...
    if (is_ssl()) {
            ...
            wp_register_script('googlemapsapi', 
                              'http://maps.googleapis.com/maps/api/js?v=3.exp&libraries=adsense,places', 
                              array('jquery'), 
                              null, 
                              false);
    }
    wp_register_script('resmap', plugins_url('includes/js/resmap.min.js', __FILE__), array('jquery'), '2.29');
    wp_register_script('resmap_admin', plugins_url('includes/js/resmap.admin.min.js', __FILE__), array('jquery'), '2.29');
    
    // Enque the neccessary scripts
    wp_enqueue_script('jquery');
    wp_enqueue_script('googlemapsapi');
    wp_enqueue_script('resmap');
    wp_enqueue_script('resmap_admin');
}
...
function resmap_shortcode($atts) {
    ...
    var mapdiv = jQuery("#responsive_map_<?php echo $mapid; ?>"); // the div that will contain the map.
    var publisherid = "<?php echo $atts['publisherid']; ?>"; // adsense publisher id.
    var clustering = "<?php echo $atts['clustering']; ?>"; // clustering?
  ...
  resmap_createSearchBox(mapdiv.data('gmap').gmap);
  <?php } ?>
