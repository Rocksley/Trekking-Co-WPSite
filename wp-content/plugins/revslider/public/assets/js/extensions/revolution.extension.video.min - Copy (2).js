			...
			case "html5":
				...
				if (html5vid.data('metaloaded') != 1) {						
					addEvent(video,'loadedmetadata',function(_nc) {								
						...
						// Here Rob
						alert("Playback Rate not changed");
						video.play(); 
						var s = getStartSec(_nc.data('videostartat'));
	...
	manageVideoLayer : function(_nc,opt,recalled,internrecalled) {				
		...
		switch (newvideotype) {
			case "html5":
				...
				var apptxt = '<video id="'+videoid+'" style="object-fit:cover;background-size:cover;visible:hidden;width:100%; height:100%" class="" '+videoloop+' preload="'+videopreload+'">'
