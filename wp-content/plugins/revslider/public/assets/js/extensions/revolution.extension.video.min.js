			...
			case "html5":
				...
				if (html5vid.data('metaloaded') != 1) {						
					addEvent(video,'loadedmetadata',function(_nc) {								
						...
						// Here Rob
						//console.log("Playback Rate not changed");
						video.play(); 
						var s = getStartSec(_nc.data('videostartat'));
					...
					// Here Rob
					if ( _nc.data('videoid') == "abc") {
						video.playbackRate = 0.5+0.25;
					//	console.log("Playback Rate =",(0.5+0.25)); 
					} else if (_nc.data('videoid') == "river") {
						video.playbackRate = 0.5/2;
					//	console.log("Playback Rate =",(0.5/2)); 
					} else if (_nc.data('videoid') == "namche") {
						video.playbackRate = 0.5-0.15;
					//	console.log("Playback Rate =",(0.5-0.15)); 
					} else if (_nc.data('videoid') == "kalapathar") {
						video.playbackRate = 0.5;
					} else {
						video.playbackRate = 0.5-0.1;
					//	console.log("Playback Rate =",(0.5-0.1)); 
					}
					video.play(); 					
					var s = getStartSec(_nc.data('videostartat'));
					if (s!=-1) video.currentTime = s;										
	...
	manageVideoLayer : function(_nc,opt,recalled,internrecalled) {				
		...
		switch (newvideotype) {
			case "html5":
				...
				var apptxt = '<video playsinline id="'+videoid+'" style="object-fit:cover;background-size:cover;visible:hidden;width:100%; height:100%" class="" '+videoloop+' preload="'+videopreload+'">'
